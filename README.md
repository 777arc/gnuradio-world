...
[`gr-rds/`](../gr-rds) and [`gr-foo/`](../gr-foo)) is not part of that umbrella
build, so there is no `libgnuradio-<m>.a` to link. Handle it as a deferred module
with two adjustments: (a) its `.block.yml` files must carry a `cpp` flag +
`cpp_templates` (bastibl's gr-rds/gr-foo ship Python-only templates; they were
added); and (b) instead of the generic `DEFERRED_MODULES` loop, add a bespoke
`add_custom_command` in [`runner/CMakeLists.txt`](runner/CMakeLists.txt) that
compiles the module's own `lib/*.cc` straight into `<m>.wasm` alongside its
generated registrar (see the `rds.wasm` / `foo.wasm` rules). Everything else —
palette, on-demand fetch, `side_exports` — works unchanged because the blocks are
in `block_module_map` and the `.wasm` is in `SIDE_MODULE_OUTPUTS`.

Practical gotchas seen wiring these two: any host-only dependency must be
compiled out or replaced under `#ifdef __EMSCRIPTEN__` — gr-rds's single
Boost.Locale call became an inline ISO-8859-2 conversion, and gr-foo's UHD
`tx_time` tagging was already behind `#ifdef FOO_UHD` (left undefined). A `raw`
PMT param the generator can't type (gr-foo's `tag_name`/`msg`) is retyped to
`string` with the `make` wrapped in `pmt.intern(...)`/`pmt::intern(...)`. Each
module also needs an empty `lib/config.h` (normally generated by its own CMake).
Python-only blocks — a GUI QWidget like `rds_panel` (`rds.rdsPanel`) or a
`gr.hier_block2` like gr-foo's `selector`/`valve` — have no C++ path and stay
