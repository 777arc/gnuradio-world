...
//
//   node wasm/test_smoke.mjs [port]
//
// Serves wasm/ itself (COOP/COEP, as SharedArrayBuffer requires) so there is no
...
const exe = existsSync(base)
  ...
  : undefined;
if (!exe) {
  console.error('chrome-headless-shell not found under wasm/chrome-headless-shell/.\n' +
                'Install it with:  npx @puppeteer/browsers install chrome-headless-shell@stable --path wasm/chrome-headless-shell');
  process.exit(2);
}
