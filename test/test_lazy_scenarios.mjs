...
const scenarios = [
  ...
  { name: 'gr-inspector analysis factories and GUI sink (OOT deferred)',
    fg: { blocks:[
      { name:'layout', id:'wasm_gui_layout', params:{
        columns:12, row_height:60, layout:JSON.stringify({ gui:[0,0,12,5] }) } },
      { name:'src', id:'analog_sig_source_x',
        params:{ type:'complex', samp_rate:48000, waveform:'cos',
                 freq:8000, amp:0.8 } },
      { name:'thr', id:'blocks_throttle2',
        params:{ type:'complex', samples_per_second:48000, vlen:1,
                 ignoretag:'True', limit:'auto', maximum:0.1 } },
      { name:'detector', id:'inspector_signal_detector_cvf',
        params:{ samp_rate:48000, fft_len:1024,
                 window:'window.WIN_HAMMING', threshold:-30,
                 sensitivity:0.2, auto:'False', avg:0.8,
                 quant:0.001, min_bw:0, logfile:'' } },
      { name:'gui', id:'inspector_qtgui_sink_vf',
        params:{ samp_rate:48000, fft_len:1024, cfreq:0, rf_unit:1,
                 msgports:1, manual:'False', gui_hint:'' } },
      { name:'separator', id:'inspector_signal_separator_c',
        params:{ samp_rate:48000, trans_width:0.1,
                 window:'window.WIN_HAMMING', oversample:1,
                 file:'False', taps_file:'' } },
      { name:'extractor', id:'inspector_signal_extractor_c',
        params:{ signal:0, resample:'False', rate:48000, osf:1 } },
      { name:'estimator', id:'inspector_ofdm_zkf_c',
        params:{ samp_rate:48000, signal:0, min_items:7000,
                 alpha:'[16, 32, 64, 128, 256, 512, 1024]',
                 beta:'[4, 8, 16, 32]' } },
      { name:'synchronizer', id:'inspector_ofdm_synchronizer_cc',
        params:{ min_items:4096 } } ],
      connections:[
        ['src',0,'thr',0], ['thr',0,'detector',0], ['detector',0,'gui',0],
        { src_blk_id:'detector', src_port_id:'map_out',
          snk_blk_id:'gui', snk_port_id:'map_in' },
      ] },
    expectFetch: ['inspector.wasm'],
    expectTile:{ name:'gui', col:0, row:0, w:12, h:5 } },
  ...
  // every other scenario here) that one not using it never does. An empty device
  // deliberately enters UHD discovery: headless Chrome has no granted radio, so
  // this also proves UHD logging reports the lookup failure without trapping at
  // the main/side-module function table boundary.
  { name: 'USRP B2xx Source (runtime_module side module)',
    fg: { blocks:[
      { name:'usrp', id:'wasm_usrp_b2xx_source',
        params:{ device:'', samp_rate:1000000, center_freq:100000000,
                 gain:30, bandwidth:0, antenna:'RX2', master_clock_rate:0 } },
      { name:'snk', id:'blocks_null_sink', params:{ type:'complex' } } ],
      connections:[ ['usrp',0,'snk',0] ] },
    expectFetch: ['b2xx.wasm'],
    expectLog: 'USRP B2xx Source: could not open device' },
];
