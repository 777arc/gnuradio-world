...
``parameter_dtypes`` / ``parameter_defaults`` / ``parameter_labels``
    ...
    likewise preserve a native ``raw`` nested numeric sequence while making the
    editor evaluate variable references and the generator render
    ``wasm_registry::matrix()``; a complex entry reaches it as a ``[re, im]``
    pair, exactly as ``complex_vector`` does.
