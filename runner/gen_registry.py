...
def generate(output: Path, manifest: Path) -> None:
    rendered = []
    includes: set[str] = set()
    ...
    for block in load_blocks():
        try:
            ...
            includes.update(block_includes)
            rendered.append(factory)
    ...
    print(f"generated {len(rendered)} factories (+ {len(CUSTOM_IDS)} custom); skipped {len(skipped)}")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--output", type=Path,
                        default=Path(__file__).with_name("src") / "generated_registry.cpp")
