...
| [docs/cgran.md](docs/cgran.md) | adding or updating an OOT on the public `/cgran/` catalog, refreshing its repository activity, or changing how project pages find examples |
...
- **Runner** (`runner/`): a generic C++/WASM "player" — parses the flowgraph
  ...
  gr-adsb, gr-tempest, gr-inspector, gr-iridium, gr-ais, gr-correctiq, gr-lte). Stream
...
palette. The example pages are rebuilt from `example_flowgraphs/` by
`npm run examples`, which `prebuild` and `predev` run for you. `npm run build` and `npm run dev` fail with a clear message rather than
