...
| [docs/ai-copilot.md](docs/ai-copilot.md) | touching Flowgraph Copilot — the shared-key proxy in `workers/ai-proxy/`, the OpenRouter and OpenAI providers, structured graph tools, the agent loop, visible-run diagnostics, consent/key storage, or hardware authorization rows |
...
| `workers/ai-proxy/` | Cloudflare Worker holding one OpenAI key for every visitor, metered per IP and per day, behind Flowgraph Copilot's default provider — see [docs/ai-copilot.md](docs/ai-copilot.md). Both Workers deploy by hand with `wrangler`; no CI deploys them |
