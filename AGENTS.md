...
Keep [`scripts/r2-cors.json`](scripts/r2-cors.json) applied to the bucket. This
is a manual step — committing the file changes nothing, and no workflow applies
it:
...
(every PR preview — see "Continuous integration") and `http://localhost:8090`,
permits GET/HEAD and Range requests, and exposes
`Content-Length`/`Content-Range`. Local testing must use
...
exactly the same steps; `deploy-wasm.yml` is now only the trigger, the inputs and
...
- **Concurrency groups are per-ref**, not the fixed `deploy-wasm` string they used
  to be, or a PR build would cancel an in-flight production deploy.
...
- **Preview origins must be in the bucket's CORS policy** or recordings break
  while everything else works — see "R2 recording source of truth".
