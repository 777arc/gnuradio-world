...
`const json&` is factory-side, a block class takes plain C++ arguments. That, the
`CUSTOM_IDS` / `INVALID_CPP_TEMPLATES` / `EXCLUDED_BLOCKS` conventions, and the
`generated_blocks.json` support manifest are in [docs/blocks.md](docs/blocks.md).
...
- **`gui_hint` does nothing; one block arranges the whole window.** Every
  ...
  `GUI_IDS` entry silently loses its tile. See
