  ...
  tool('exercise_js_block', 'Run bounded deterministic calls against a JS Block in a disposable Worker using the real JS runtime contract. Use before a visible run, especially for new or repaired code: unlike a live scheduler thread, this worker is terminated on a hang.', object({
    ...
    calls: { type: 'array', minItems: 0, maxItems: 8,
      ...
      items: object({
        ...
        inputs: { type: 'array', description: 'One finite-number array per input port, repeated (tiled) to fill the call: a 1024-item complex window of 1+0j is [[1, 0]], a float ramp of period 4 is [[0, 1, 2, 3]]. Write a pattern once, never a long literal array. Omitted ports are zero-filled.', items: { type: 'array', items: { type: 'number' } } },
...
const JS_HELP: Record<string, string> = {
  ...
  debugging: `Use this.log(...) for the editor console; console.log from a scheduler worker reaches only devtools. Exercise candidate code with small deterministic arrays, input tags and messages before a live run because a live callback that never returns cannot be interrupted. A short input array is tiled to the call's length, so a whole 1024-sample window of a constant is inputs: [[1, 0]] with nout: 1024 -- never type out a long array. The result includes outputs, consumed counts, messages_published and tags_added. Throughput proves progress, not signal correctness: use known exercise outputs or a Probe in the visible graph.`,
