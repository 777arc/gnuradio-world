...
export const AI_TOOLS: ToolDefinition[] = [
  ...
  tool('read_plot_data', 'Read what the running flowgraph\'s GUI sinks are plotting, as numbers: each plot\'s axis titles and displayed range, and per trace its peak (x and y), min/max/mean and a decimated set of points. Spectrum Analyzer results also include the exact numeric measurements behind its signal annotations, without display formatting or rounding. This is the precise and cheap way to answer "where is the peak", "what is the level", "is the tone at the right frequency" — prefer it over a screenshot for anything measurable. Needs a flowgraph that is still running.', object({
