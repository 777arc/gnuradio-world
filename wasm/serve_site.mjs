#!/usr/bin/env node
// Serves an assembled site (wasm/scripts/assemble-site.mjs) the way Cloudflare
// Pages serves it, so deploy-only behaviour can be checked without a deploy:
//
//   node wasm/scripts/assemble-site.mjs ./site && node wasm/serve_site.mjs ./site
//
...
// files are served from wasm/example_recordings (on Pages they come from R2),
// with range support, so an assembled site is fully clickable here.
//
// Usage: node wasm/serve_site.mjs [siteDir] [port]
